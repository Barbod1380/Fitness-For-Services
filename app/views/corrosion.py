...
def compute_corrosion_metrics_for_dataframe(defects_df, joints_df, pipe_diameter_mm, smys_mpa, safety_factor, maop_mpa=0.0):
    ...
    for idx, row in defects_df.iterrows():
        ...
        try:
            effective_area_result = calculate_rstreng_level1(
...
def render_corrosion_assessment_view():
    ...
    with st.expander("ℹ️ About the FFS Assessment Methods", expanded=False):
        st.markdown("""
        This tool uses established Fitness-for-Service (FFS) standards to assess corrosion defects. Here are the key assumptions for each method:

        - **B31G (Original):**
            - **Profile:** Assumes a parabolic shape for the defect.
            - **Flow Stress:** `1.1 * SMYS`. A conservative, older definition.
            - **Applicability:** Best suited for initial, conservative screening. The calculation has been updated to use the standard Folias factor formula.

        - **Modified B31G (0.85dL):**
            - **Profile:** Approximates the defect area as 85% of a corresponding rectangle (`0.85 * length * depth`). This is generally more accurate than original B31G.
            - **Flow Stress:** `SMYS + 10,000 psi (68.95 MPa)`. A more realistic definition of the material's yield behavior.
            - **Applicability:** A widely accepted industry standard providing a good balance of accuracy and conservatism.

        - **RSTRENG (Simplified):**
            - **Profile:** This is a Level-1 assessment identical to Modified B31G, using the `0.85dL` area definition. It does *not* use the full river-bottom profile (Level-2).
            - **Flow Stress:** `SMYS + 10,000 psi (68.95 MPa)`.
            - **Applicability:** Provides a modern and reliable assessment for single defects. For interacting defects (clusters), a Level-2 analysis is recommended.

        **Estimated Repair Factor (ERF)** is calculated as `MAOP / Safe Pressure`. An ERF greater than 1.0 indicates the defect's safe operating pressure is below the maximum allowable operating pressure, signaling a need for repair or pressure reduction.
        """)
